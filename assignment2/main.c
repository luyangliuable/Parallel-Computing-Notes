...
void log_to_file(double time, int no_of_alerts_detected, seismic_reading reading, int *no_of_messages, int size);
  ...
  void *thread_recv(void *vargs) {
    ...
    int tmp = 1;
    MPI_Recv(params->reading_ptr, 1, params->MPI_SEISMIC_READING,
  ...
  int main(int argc, char **argv) {
    ...
    if (rank != 0) {
      ...
      earthquake_detection_system(rank, size - 1, MPI_COMM_WORLD, new_comm,
                                  dims, msg_count, count_buffer);
  ...
  void periodic_detection(int *coord, int ndims, int *dims, int my_rank,
                          int size, int *neighbour_ranks, MPI_Comm comm2D) {
  ...
  void log_to_file(double time, int no_of_alerts_detected, seismic_reading reading,
                   int *no_of_messages, int size) {
    /**
    ...
    /* printf("Time taken %.2f, count buffer %i, no of alerts %i, size, size %i.\n", time, no_of_messages[2], no_of_alerts_detected, size); */
