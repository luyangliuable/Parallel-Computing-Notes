...
int main(int argc, char *argv[]) {
  ...
  if (argc == 4) {
    ...
    nrows = ncols = nlayers = (int) pow(size, 1/3);
  ...
  printf("Current cartesian rank is %i.\n", my_cart_rank);
  printf("Current coordinates are (%i, %i, %i).\n", coord[0], coord[1],
         coord[2]);
  printf("Process rank in front is %i, process rank behind is %i.\n", nbr_i_hi,
         nbr_i_lo);
  printf(
      "Process rank to the right is %i, process rank to the left is is %i.\n",
      nbr_j_hi, nbr_j_lo);
  printf("Process rank above is %i, process rank below is %i.\n", nbr_k_hi,
         nbr_k_lo);
