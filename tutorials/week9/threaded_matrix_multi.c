...
// file. 2D serial approach. Apply parallel sub matrix using POSIX threads.
//
//
...
void *MatrixMulFunc(void *pArg) {
  ...
  /* threadColDiv = colC / 2; */
  threadColDiv = sqrt(NUM_THREADS);
  threadRowDiv = sqrt(NUM_THREADS);
  /* threadColDivRemain = colC % 2; */
  /* threadRowDiv = rowC / (NUM_THREADS / 2); */
  /* threadRowDivRemain = rowC % (NUM_THREADS / 2); */
  int which_row = threadId / threadColDiv;
  int which_col = threadId % threadColDiv;

  // Which row is the thread number over the total number of column divisions.
  // Which column in row is the remainder of thread number / row

  // rowC/threadRowDiv gives the number of element each row division.
  row_start_point = which_row * (rowC / threadRowDiv);
  row_end_point = row_start_point + (rowC / threadRowDiv);

  // rowC/threadColDiv gives the number of element each col division.
  col_start_point = which_col * (colC / threadColDiv);
  col_end_point = col_start_point + (colC / threadColDiv);
...
int main() {
  ...
  // 1. Read Matrix A
  printf("Matrix Multiplication using 2-Dimension Arrays (POSIX - Row "
         "Partition) - Start\n\n");
  ...
  for (i = 0; i < rowC; i++) {
    ppMatrixC[i] =
        (unsigned long long *)calloc(colC, sizeof(unsigned long long));
  }
